    ...
    <div
        ...
        <div class="container-fluid group-data-[content=boxed]:max-w-boxed mx-auto">
            ...
            <div class="flex flex-col gap-2 py-4 md:flex-row md:items-center print:hidden">
                <div class="grow">
                    <h5 class="text-16">Promocode List</h5>
                </div>
                        ...
                        <div class="card-body">
                            <div class="grid grid-cols-1 gap-5 mb-5 xl:grid-cols-0">
                                ...
                                <div class="rtl:md:text-start">
                                    ...
                                    @endif
                                    <a href="{{ route('promocode.add', $id) }}" style="float: inline-end;">
                                        <button type="submit" title="Add New Promocode"
                                            class="text-white transition-all duration-200 ease-linear btn bg-custom-500 border-custom-500 hover:text-white hover:bg-custom-600 hover:border-custom-600 focus:text-white focus:bg-custom-600 focus:border-custom-600 focus:ring focus:ring-custom-100 active:text-white active:bg-custom-600 active:border-custom-600 active:ring active:ring-custom-100 dark:ring-custom-400/20"
                                            data-modal-target="AddModal">
                                            <i class="ri-add-box-fill"></i> Add New Promocode
                                        </button>
                                    </a>
                                </div>
                            </div>
                            ...
                            <div class="overflow-x-auto">
                                ...
                                @if (!$offers->isEmpty())
                                    <form id="bulkDeleteForm" method="POST"
                                        action="{{ route('promocode.deleteselected') }}">
                                        @csrf
                                        @method('DELETE')
                                                                ...
                                                                <div
                                                                    ...
                                                                    @if ($offer->iStatus == 1)
                                                                        <input type="checkbox" name="greenIconSwitch"
                                                                            id="greenIconSwitch{{ $offer->promo_id }}"
                                                                            ...
                                                                            onchange="updateStatus(<?= $offer->iStatus ?>,<?= $offer->promo_id ?>);"
                                                                            checked="">
                                                                    @else
                                                                        <input type="checkbox" name="greenIconSwitch"
                                                                            id="greenIconSwitch{{ $offer->promo_id }}"
                                                                            ...
                                                                            onchange="updateStatus(<?= $offer->iStatus ?>,<?= $offer->promo_id ?>);">
                                                                    @endif
                                                                    <label for="greenIconSwitch{{ $offer->promo_id }}"
                                                            ...
                                                            <div class="gap-2">
                                                                <div class="edit">
                                                                    <a class="mx-1" title="Edit"
                                                                        href="{{ route('promocode.edit', $offer->promo_id) }}">
                                                                        <i class="ri-edit-2-fill"></i>
                                                                    </a>

                                                                    <a class="mx-1" title="Delete" href="#"
                                                                        onclick="confirmSingleDelete({{ $offer->promo_id }})">
    ...
    <script>
        ...
        function confirmSingleDelete(id) {
            Swal.fire({
                ...
                if (result.value) {
                    fetch(`{{ route('promocode.delete', ['id' => '__id__']) }}`.replace('__id__', id), {
                            method: 'DELETE',
                            headers: {
                        ...
                        .then(data => {
                            if (data.success) {
                                Swal.fire({
                                    ...
                                    window.location.href = `{{ route('promocode.index', $id) }}`;
                                });
                            } else {
                                Swal.fire({
                                    title: 'Error!',
