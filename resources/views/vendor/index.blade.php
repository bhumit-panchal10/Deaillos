...
@section('content')
    ...
    <div
        ...
        <div class="container-fluid group-data-[content=boxed]:max-w-boxed mx-auto">
            ...
            <div class="flex flex-col gap-2 py-4 md:flex-row md:items-center print:hidden">
                <div class="grow">
                    <h5 class="text-16">Vendor List</h5>
                </div>
                        ...
                        <div class="card-body">
                            <div class="grid grid-cols-1 gap-5 mb-5 xl:grid-cols-0">
                                ...
                                <div class="rtl:md:text-start">
                                    ...
                                    @endif
                                    <a href="{{ route('vendor.add') }}" style="float: inline-end;">
                                        <button type="submit" title="Add New Vendor"
                                            class="text-white transition-all duration-200 ease-linear btn bg-custom-500 border-custom-500 hover:text-white hover:bg-custom-600 hover:border-custom-600 focus:text-white focus:bg-custom-600 focus:border-custom-600 focus:ring focus:ring-custom-100 active:text-white active:bg-custom-600 active:border-custom-600 active:ring active:ring-custom-100 dark:ring-custom-400/20"
                                            data-modal-target="AddModal">
                                            <i class="ri-add-box-fill"></i> Add New Vendor
                                        </button>
                                    </a>
                                </div>
                            </div>
                            ...
                            <div class="overflow-x-auto">
                                ...
                                @if (!$vendors->isEmpty())
                                    <form id="bulkDeleteForm" method="POST" action="{{ route('vendor.deleteselected') }}">
                                        @csrf
                                        @method('DELETE')
                                        <table class="w-full whitespace-nowrap" id="customerTable">
                                            <thead class="bg-slate-100 dark:bg-zink-600">
                                                <tr>
                                                    ...
                                                    {{-- <th class="sort px-3.5 py-2.5 font-semibold border-b border-slate-200 dark:border-zink-500 "
                                                        data-sort="state_name">Facebook Link </th> --}}

                                                    {{-- <th class="sort px-3.5 py-2.5 font-semibold border-b border-slate-200 dark:border-zink-500 "
                                                        data-sort="state_name">Instragram Link </th> --}}
                                            ...
                                            <tbody class="list form-check-all">
                                                ...
                                                @foreach ($vendors as $vendor)
                                                    <tr class="text-center">
                                                        ...
                                                        <td
                                                            class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 imageURL">
                                                            @if ($vendor->vendorimg)
                                                                <img class="img-fluid"
                                                                    src="{{ asset('upload/vendor/' . $vendor->vendorimg) }}"
                                                                ...
                                                                <div
                                                                    ...
                                                                    @if ($vendor->iStatus == 1)
                                                                        <input type="checkbox" name="greenIconSwitch"
                                                                            id="greenIconSwitch{{ $vendor->vendor_id }}"
                                                                            ...
                                                                            onchange="updateStatus(<?= $vendor->iStatus ?>,<?= $vendor->vendor_id ?>);"
                                                                            checked="">
                                                                    @else
                                                                        <input type="checkbox" name="greenIconSwitch"
                                                                            id="greenIconSwitch{{ $vendor->vendor_id }}"
                                                                            ...
                                                                            onchange="updateStatus(<?= $vendor->iStatus ?>,<?= $vendor->vendor_id ?>);">
                                                                    @endif
                                                                    <label for="greenIconSwitch{{ $vendor->vendor_id }}"
                                                            ...
                                                            <div class="gap-2">
                                                                <div class="edit">
                                                                    <a class="mx-1" title="Edit"
                                                                        href="{{ route('vendor.edit', $vendor->vendor_id) }}">
                                                                        <i class="ri-edit-2-fill"></i>
                                                                    </a>

                                                                    <a class="mx-1" title="Delete" href="#"
                                                                        onclick="confirmSingleDelete({{ $vendor->vendor_id }})">
                                                                        <i class="ri-delete-bin-5-fill"></i>
                                                                    </a>

                                                                    <a class="mx-1" title="Add Deal"
                                                                        href="{{ route('deal.index', $vendor->vendor_id) }}">
    ...
    <script>
        function checkAll(source) {
            checkboxes = document.querySelectorAll('input[name="vendor_ids[]"]');
        ...
        function confirmSingleDelete(id) {
            Swal.fire({
                ...
                if (result.value) {
                    fetch(`{{ route('vendor.delete', ['id' => '__id__']) }}`.replace('__id__', id), {
                            method: 'DELETE',
                            headers: {
    ...
    <script>
        function updateStatus(status, vendorId) {
            //$("p").toggle();
