                ...
                <div class="card" id="customerList">
                    ...
                    <div class="card-body">
                        ...
                        <div class="grid grid-cols-1 gap-5 mb-5 xl:grid-cols-0">
                            ...
                            <div class="rtl:md:text-start">
                                ...
                                <a class="mx-1 ltr:md:text-end" title="Add New Career"
                                    href="{{ route('deal.add', $id) }}" style="float: inline-end;">
                                    <button type="submit"
                                        class="text-white transition-all duration-200 ease-linear btn bg-custom-500 border-custom-500 hover:text-white hover:bg-custom-600 hover:border-custom-600 focus:text-white focus:bg-custom-600 focus:border-custom-600 focus:ring focus:ring-custom-100 active:text-white active:bg-custom-600 active:border-custom-600 active:ring active:ring-custom-100 dark:ring-custom-400/20">
                                        <i class="ri-add-box-fill"></i> Add Deal
                                    </button>
                                </a>
                            </div>
                        </div>
                        ...
                        <div class="overflow-x-auto">
                            @if (!$deals->isEmpty())
                            <form id="bulkDeleteForm" method="POST" action="{{ route('deal.deleteselected') }}">
                                @csrf
                                @method('DELETE')
                                                ...
                                                <div class="flex gap-2">
                                                    <div class="edit">
                                                        <a class="mx-1" title="Edit"
                                                            href="{{ route('deal.edit', $deal->Deals_id) }}">
                                                            <i class="ri-edit-2-fill"></i>
                                                        </a>

                                                        <a class="mx-1" title="Delete" href="#"
                                                            onclick="confirmSingleDelete({{ $deal->Deals_id }})">
...
<script>
    function checkAll(source) {
        checkboxes = document.querySelectorAll('input[name="deal_ids[]"]');
        checkboxes.forEach(checkbox => checkbox.checked = source.checked);
    }

    function confirmBulkDelete() {
        const selected = document.querySelectorAll('input[name="deal_ids[]"]:checked');
    ...
    function confirmSingleDelete(id) {
        Swal.fire({
            ...
            if (result.value) {
                // Correctly generate the URL with the ID dynamically
                const url = `{{ route('deal.delete', ['id' => '__id__']) }}`.replace('__id__', id);
                ...
                fetch(url, {
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            Swal.fire({
                                title: 'Error!',
                                text: data.message || 'There was an issue deleting the deal.',
                                icon: 'error',
                                customClass: {
                                    confirmButton: 'text-white btn bg-custom-500 border-custom-500 hover:text-white hover:bg-custom-600 hover:border-custom-600 focus:text-white focus:bg-custom-600 focus:border-custom-600 focus:ring focus:ring-custom-100 active:text-white active:bg-custom-600 active:border-custom-600 active:ring active:ring-custom-100 dark:ring-custom-400/20',
                                },
