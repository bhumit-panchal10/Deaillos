...
@section('title', 'Header Image List')
@section('content')
    ...
    <div
        ...
        <div class="container-fluid group-data-[content=boxed]:max-w-boxed mx-auto">
            ...
            <div class="flex flex-col gap-2 py-4 md:flex-row md:items-center print:hidden">
                <div class="grow">
                    <h5 class="text-16">Header Image List</h5>
                </div>
                            ...
                            <div class="grid grid-cols-1 gap-5 mb-5 xl:grid-cols-0">
                                ...
                                <div class="rtl:md:text-start">
                                    ...
                                    <a class="mx-1 ltr:md:text-end" title="Add New Career"
                                        href="{{ route('headerimage.add') }}" style="float: inline-end;">
                                        <button type="submit"
                                            class="text-white transition-all duration-200 ease-linear btn bg-custom-500 border-custom-500 hover:text-white hover:bg-custom-600 hover:border-custom-600 focus:text-white focus:bg-custom-600 focus:border-custom-600 focus:ring focus:ring-custom-100 active:text-white active:bg-custom-600 active:border-custom-600 active:ring active:ring-custom-100 dark:ring-custom-400/20">
                                            <i class="ri-add-box-fill"></i> Add Header Image
                                        </button>
                                    </a>
                                </div>
                            </div>
                            ...
                            <div class="overflow-x-auto">
                                @if (!$headerimage->isEmpty())
                                    <form id="bulkDeleteForm" method="POST"
                                        action="{{ route('headerimage.deleteselected') }}">
                                        @csrf
                                        @method('DELETE')
                                                ...
                                                @foreach ($headerimage as $img)
                                                    <tr>
                                                        ...
                                                        <td
                                                            ...
                                                            @if ($img->header_image)
                                                                <img class="img-fluid"
                                                                    src="{{ asset('upload/headerImage') . '/' . $img->header_image }}"
                                                            ...
                                                            <div class="flex gap-2">
                                                                <div class="edit">
                                                                    <a class="mx-1" title="Edit"
                                                                        href="{{ route('headerimage.edit', $img->header_image_id) }}">
                                                                        <i class="ri-edit-2-fill"></i>
                                                                    </a>

                                                                    <a class="mx-1" title="Delete" href="#"
                                                                        onclick="confirmSingleDelete({{ $img->header_image_id }})">
                                    ...
                                    <div class="flex items-center justify-between mt-5">
                                        {{ $headerimage->appends(request()->except('page'))->links() }}
                                    </div>
                                @else
                                    <div class="noresult">
    ...
    <script>
        function checkAll(source) {
            checkboxes = document.querySelectorAll('input[name="headerimage_ids[]"]');
            checkboxes.forEach(checkbox => checkbox.checked = source.checked);
        }

        function confirmBulkDelete() {

            const selected = document.querySelectorAll('input[name="headerimage_ids[]"]:checked');
        ...
        function confirmSingleDelete(id) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    fetch('{{ route('headerimage.delete', ['id' => '__id__']) }}'.replace('__id__', id), {
                            method: 'DELETE',
                            headers: {
